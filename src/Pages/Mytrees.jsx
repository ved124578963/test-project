...
const MyTrees = () => {
  ...
  useEffect(() => {
    ...
    const fetchTrees = async () => {
      try {
        const response = await axios.get("http://localhost:8080/api/treeowner/my-trees", {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {trees.map((tree) => (
            <div key={tree.id} className="bg-white p-4 rounded-lg shadow-md">
              ...
              <img src={tree.photoUrl} alt="Tree" className="w-full h-40 object-cover rounded mt-2" />
